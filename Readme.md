...
- ~~`ENV_DELETE_TOPIC_AS_BAN`：删除话题视为永久封禁（true/false，不区分大小写），默认为 false。false 时只会删除话题，对方只需要再发送一次消息即可再次新建话题~~ （已弃用）
...
2. **管理命令**（命令不会被转发给对方）
   ...
   - `/checkblock`：检查用户屏蔽状态（在相应话题内直接使用），展示屏蔽状态列表（在 general 话题中使用，包含验证码错误超过设定次数的被屏蔽者）
...
3. **话题管理**（另一种封禁/屏蔽方式，更便捷，无需输入）
   ...
   - 删除话题：删除话题后，对方只需要额外进行一次发送信息即可重新创建话题，类似clear命令

PS：想要永久屏蔽并让话题被删除（清除），请使用同一类命令，如`/block`后`/clear`，或关闭话题后删除话题。
